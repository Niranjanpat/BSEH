...
import { getUserTadas } from '../../../services/user_tadas_service'; 
...
const UserTadasListScreen = () => {
  const today = new Date();
  const [tadas, setTadas] = useState([]);
  const page = useRef(1);
  ...
  const fetchExpenses = async (currentPage) => {
    ...
    try {
      ...
      console.log('Fetch Tadas Response:', data);
      if (success) {
        const newTadas = data.ta_das || [];
        setTadas((prev) =>
          currentPage === 1 ? newTadas : [...prev, ...newTadas]
        );
    ...
    <View style={{ flex: 1 }}>
      ...
      {tadas.length ? (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TadasCard item={item} navigation={navigation} />
          )}
  ...
  return (
    <View style={[styles.card, { borderLeftColor: getStatusColor(item.status) }]}>
      ...
      >
        ...
        <Text style={styles.text}>Type: {item.expense_type ?? 'N/A'}</Text>
...
export default UserTadasListScreen;
